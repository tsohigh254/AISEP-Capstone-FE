...
// Live recommendations will be fetched from backend
    ...
    GetRecommendations(10)
      .then((res) => {
        ...
        const mapped = matches.map((m: any, idx: number) => {
          ...
          const name = m.startupName ?? m.StartupName ?? "Unknown";
          const score = Math.round(m.finalMatchScore ?? m.FinalMatchScore ?? 0);
          const matchReason =
            m.fitSummaryLabel ?? m.FitSummaryLabel ?? (m.matchReasons ? (m.matchReasons as string[]).join("; ") : "");
          const img = m.logoUrl ?? m.logoURL ?? `https://via.placeholder.com/160?text=${encodeURIComponent(name)}`;
          return {
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {loading ? (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-neutral-surface">
          <p className="text-sm text-neutral-muted">Đang tải gợi ý từ AI…</p>
        </div>
      ) : error ? (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-neutral-surface">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {recs.length === 0 ? (
            <div className="col-span-1 text-sm text-neutral-muted">Không có gợi ý</div>
          ) : (
            recs.map((rec) => (
