...
export default function StartupInfoPage() {
    const { form, updateForm, logoFile, setLogoFile, profileLogoURL, setProfileLogoURL, certificateFile, setCertificateFile, loading } = useStartupProfile();
    const [industries, setIndustries] = useState<IIndustryFlat[]>([]);
    const fileRef = useRef<HTMLInputElement>(null);
    const certFileRef = useRef<HTMLInputElement>(null);
    // Gom toàn bộ create/update ở 1 trang (/info). Member tách ở tab (/team).
                ...
                {activeTab === "overview" && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                        ...
                        <div className="flex flex-col md:flex-row gap-8 items-start">
                            ...
                            <div className="shrink-0 flex flex-col gap-3">
                                <label className={labelCls}>Giấy ĐKKD <span className="text-red-500">*</span></label>
                                <div
                                ...
                                >
                                    {certificateFile ? (
                                        <div className="text-center p-2">
                                            <p className="text-[11px] font-medium text-[#0f172a] truncate w-24" title={certificateFile.name}>
                                                {certificateFile.name}
                                            </p>
                                        </div>
                                    ) : (
                                        <div className="text-center">
                                            <Camera className="w-8 h-8 text-slate-300 mx-auto mb-1 group-hover:scale-110 transition-transform" />
                                            <span className="text-[10px] font-semibold text-slate-400">TẢI LÊN</span>
                                        </div>
                                    )}
                                    <div className="absolute inset-0 bg-[#0f172a]/60 opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center text-white gap-1 backdrop-blur-sm">
                                        <Camera className="w-5 h-5" />
                                        <span className="text-[10px] font-semibold tracking-wide">THAY FILE</span>
                                    </div>
                                </div>
                                {certificateFile && (
                                    <button
                                ...
                                <input ref={certFileRef} type="file" accept=".pdf,image/*" className="hidden" onChange={handleCertFileChange} />
                            </div>
                                ...
                                <div className="relative">
                                    ...
                                    <NumericFormat
                                        ...
                                        isAllowed={({ floatValue }: { floatValue?: number }) =>
                                            !floatValue || floatValue <= 999999999999
                                        }
                                        value={form.fundingAmountSought || ""}
                                        onValueChange={({ value }: { value: string }) =>
                                            updateForm("fundingAmountSought", value)
                                        }
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <NumericFormat
                                        ...
                                        onValueChange={({ value }: { value: string }) =>
                                            updateForm("currentFundingRaised", value)
                                        }
                                        className={cn(inputCls, "pl-8 text-[15px] font-semibold tracking-wide")}
                                        placeholder="250,000"
                                    />
                ...
                {true && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label className={labelCls}>Họ tên người đăng ký</label>
                                <input
                            ...
                            <div>
                                <label className={labelCls}>Vai trò người đăng ký</label>
                                <input
