...
export default function StartupTeamPage() {
    ...
    const fetchMembers = async () => {
        ...
        try {
            ...
            if ((res.success || res.isSuccess) && Array.isArray(res.data)) {
                setMembers(res.data);
            } else {
                setMembers([]);
            }
    ...
    const handleOpenEditDialog = (member: ITeamMember) => {
        resetForm();
        setIsEditing(true);
        setCurrentMemberId(member.teamMemberID);
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden pb-20">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
                ...
                <Button onClick={handleOpenAddDialog} variant="outline" className="border-yellow-200 bg-yellow-50/30 text-yellow-700 hover:bg-yellow-50 gap-2 shrink-0 h-11">
            ...
            <div className="divide-y divide-gray-50">
                {isLoading ? (
                    ...
                    <div className="py-12 text-center text-sm text-gray-500">Chưa có thành viên nào. Hãy thêm thành viên đầu tiên!</div>
                ) : (
                    members.map((member: ITeamMember) => (
                        <div key={member.teamMemberID} className="grid grid-cols-12 gap-4 px-8 py-6 items-center hover:bg-gray-50/50 transition-colors">
                            <div className="col-span-6 flex items-start gap-4">
                                ...
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        {member.isFounder && (
                                            <span className="px-2 py-0.5 rounded-full bg-yellow-400 text-yellow-900 text-[10px] font-bold uppercase tracking-wide">FOUNDER</span>
                                        )}
                                    </div>
                                    <p className="text-sm text-gray-500 leading-snug line-clamp-2 max-w-sm">{member.bio}</p>
                                </div>
                            </div>
                            ...
                            <div className="col-span-1 flex justify-end gap-2">
                                <button onClick={() => handleOpenEditDialog(member)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                                    <Pencil className="w-4 h-4" />
                                </button>
                                <button onClick={() => handleDelete(member.teamMemberID)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
                    <DialogHeader className="border-b pb-4 mb-4">
                        <DialogTitle className="text-xl font-bold">{isEditing ? "Cập nhật thành viên" : "Thêm thành viên mới"}</DialogTitle>
                    </DialogHeader>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="col-span-2 mt-2 space-y-1.5">
                                ...
                                <div className="flex flex-col items-center gap-3 mt-2">
                                    <div className="relative w-32 h-32 rounded-xl border-2 border-dashed border-gray-200 bg-gray-50/50 flex items-center justify-center overflow-hidden group transition-colors hover:border-blue-300">
                                        {photoFile ? (
                                            <img src={URL.createObjectURL(photoFile)} alt="Avatar preview" className="w-full h-full object-cover" />
                                        ) : existingPhotoURL ? (
                                    ...
                                    <label htmlFor="photo" className="inline-flex items-center gap-1.5 cursor-pointer px-3 py-1.5 text-sm font-medium text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors">
