...
export function InvestorPublicProfileModal({
  ...
  const presentation = profile ? buildInvestorProfilePresentation(profile) : null;
  const displayName = presentation?.primaryName || profile?.fullName || profile?.firmName || "Nhà đầu tư";
  const investorTypeLabel = presentation?.categoryLabel || profile?.investorType || "Nhà đầu tư";
  const preferredStages = profile?.preferredStages ?? [];
  const preferredIndustries = profile?.preferredIndustries ?? [];

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto rounded-2xl border-none p-0 shadow-2xl sm:max-w-2xl">
        <div className="relative">
          <div className="h-32 rounded-t-2xl bg-gradient-to-r from-slate-900 to-slate-800" />
          <div className="absolute -bottom-10 left-6 flex h-20 w-20 items-center justify-center overflow-hidden rounded-2xl border-4 border-white bg-slate-900 shadow-lg relative">
            {profile?.profilePhotoURL ? (
              <Image src={profile.profilePhotoURL} alt={displayName} fill sizes="80px" className="object-cover" />
            ) : (
        ...
        {!profile ? (
          ...
          <div className="space-y-6 px-6 pb-6 pt-14">
            <DialogHeader className="text-left">
              <div className="flex flex-wrap items-center gap-3">
                <div className="flex items-center gap-2">
                  <DialogTitle className="text-2xl font-bold text-[#171611]">{displayName}</DialogTitle>
            ...
            {profile.bio && (
              <div>
                <h4 className="mb-2 font-be-vietnam text-xs font-black uppercase tracking-widest text-neutral-400">Giới thiệu</h4>
                <p className="font-be-vietnam text-sm leading-relaxed text-[#171611]">{profile.bio}</p>
              </div>
            )}

            {profile.investmentThesis && (
              <div>
                <h4 className="mb-2 font-be-vietnam text-xs font-black uppercase tracking-widest text-neutral-400">Khẩu vị đầu tư</h4>
                <p className="font-be-vietnam text-sm leading-relaxed text-[#171611]">{profile.investmentThesis}</p>
              </div>
            )}
            ...
            {preferredIndustries.length > 0 && (
              <div>
                <h4 className="mb-3 font-be-vietnam text-xs font-black uppercase tracking-widest text-neutral-400">Lĩnh vực quan tâm</h4>
                <div className="flex flex-wrap gap-2">
            ...
            <div className="flex items-center justify-between rounded-xl border border-slate-800 bg-slate-900 p-4 text-white">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.1em] text-slate-400">Trạng thái kết nối</p>
