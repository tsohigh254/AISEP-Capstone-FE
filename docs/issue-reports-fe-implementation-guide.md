...
```ts
{
  ...
  reporterEmail: string | null;   // BE entity User chưa có FirstName/LastName → field này là email, có thể null
...
**Lưu ý display name**: Field `reporterEmail` là email, không phải tên người. UI staff detail page chỉ show email — KHÔNG dùng làm display name (vd "Nguyễn Văn A"). Nếu cần resolve display name, query bảng khác (Startup/Advisor/Investor profile theo `reporterUserID`).
...
export interface IssueReportDetailDto extends IssueReportSummaryDto {
  reporterUserID: number;
  reporterEmail: string | null;   // KHÔNG phải tên người — là email do BE chưa có FirstName/LastName
  staffNote: string | null;
  assignedToStaffID: number | null;
  ...
  return axios.get<IBackendRes<IPaginatedRes<IssueReportSummaryDto>>>(
    `/api/issue-reports/me`,
    {
      params: {
        ...
        // Dùng != null thay vì truthy check — defensive, đề phòng signature đổi sang number sau này (0 là falsy)
...
export const UpdateIssueReportStatus = (
  id: number,
  body: { status: IssueReportStatus; staffNote?: string }   // KHÔNG cho phép null — chỉ string hoặc undefined
) => {
  return axios.patch<IBackendRes<IssueReportDetailDto>>(
    `/api/issue-reports/${id}/status`,
    {
      ...
      // staffNote semantics:
      //   undefined (omit)        → BE giữ note cũ
      //   "" (empty string)       → BE overwrite thành "" (xóa note)
      //   "text"                  → BE overwrite thành "text"
      //   null                    → KHÔNG dùng (BE coi như undefined → giữ note cũ, dễ nhầm)
...
> **`staffNote` semantics — đọc kỹ trước khi build staff detail UI**
>
> BE source (`report.StaffNote = request.StaffNote ?? report.StaffNote`):
>
> | FE gửi | DB sau PATCH |
> |---|---|
> | field absent (`undefined`) | giữ note cũ |
> | `null` | giữ note cũ (KHÔNG xóa) |
> | `""` | overwrite thành `""` (xóa note) |
> | `"text"` | overwrite thành `"text"` |
>
> → Type signature `staffNote?: string` ở trên đã enforce không thể pass `null`. **Tuân thủ convention này khi gọi function**.
...
- Hiển thị: reporter **email** (KHÔNG show làm display name — xem §3), category, status (current), description, attachments (link click mở Cloudinary URL trong tab mới), staffNote, submittedAt/updatedAt
- 4 button đổi status (`NEW` / `UNDER_REVIEW` / `RESOLVED` / `DISMISSED`) — disable button của status hiện tại
- Textarea `staffNote` pre-fill với note cũ (nếu có)
...
#### staffNote dirty tracking (CRITICAL)

Pre-fill textarea bằng note cũ là OK về UX, nhưng **không được luôn gửi** giá trị textarea lên BE — sẽ overwrite note cũ bằng chính nó (no-op nhưng dirty `updatedAt` sai). Phải track xem user đã thực sự chỉnh note chưa:

```ts
const [note, setNote] = useState(initialReport.staffNote ?? "");
const [noteDirty, setNoteDirty] = useState(false);

// Trong onChange textarea: setNote(v); setNoteDirty(true);

const handleSave = async (newStatus: IssueReportStatus) => {
  await UpdateIssueReportStatus(id, {
    status: newStatus,
    ...(noteDirty && { staffNote: note }),   // chỉ gửi khi user đã chạm
  });
  // Sau khi save, refresh data và reset noteDirty
};
```

3 case test phải pass:
| Action của staff | Body FE gửi | DB sau PATCH |
|---|---|---|
| Click button đổi status, không chạm textarea | `{ status: 2 }` | note cũ giữ nguyên |
| Xóa trắng textarea, click save | `{ status: 2, staffNote: "" }` | note thành `""` |
| Sửa textarea thành text mới, click save | `{ status: 2, staffNote: "text mới" }` | note = "text mới" |

### Acceptance Phase 2
- [ ] Staff thấy report user vừa submit ở Phase 1 (không còn mock data IS-5001)
- [ ] Filter status `New` → chỉ thấy report mới (request có `?status=0`)
- [ ] Click row → vào detail, thấy đủ attachment + reporter email (KHÔNG hiện làm "Nguyễn Văn A")
- [ ] Đổi status `New → UnderReview` + chỉnh note → reload thấy note mới
- [ ] Đổi status, KHÔNG chạm textarea → request omit `staffNote` → note cũ giữ nguyên
- [ ] Xóa trắng textarea + save → request gửi `staffNote: ""` → DB note thành empty
- [ ] Click attachment → mở Cloudinary URL trong tab mới
- [ ] Reporter mở `/staff/issue-reports/{id}` → 403 → redirect về list
