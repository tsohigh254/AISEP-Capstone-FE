...
- Dùng `GetAdvisorMentorshipById` từ `services/advisor/advisor.api.ts`
- Không tạo thêm function duplicate chỉ cho staff nếu vẫn gọi cùng endpoint `/api/mentorships/{id}`
- Nếu sau này muốn sạch kiến trúc hơn thì move sang shared mentorship service, nhưng không phải scope của task này
...
- Không tự check report pass ở FE để suy ra payout eligibility
...
- Không check riêng `completionConfirmedByStartup`
- `isPayoutEligible` là single source of truth cho eligibility tổng; BE đã tính từ session confirm + report review state + các điều kiện nội bộ khác
...
### 4f. Đồng bộ lại list sau khi release

List page hiện được thiết kế theo hướng fetch một lần khi mount. Vì vậy sau khi release payout ở detail page, nếu user quay lại list mà không có invalidation thì item vẫn nằm ở tab `Chờ giải ngân`.

FE phải chọn một trong 2 cách:

- gọi `router.refresh()` khi điều hướng quay về list sau release thành công
- hoặc có cơ chế invalidate/refetch list state khi quay lại screen list

Nguyên tắc:

- không được assume back navigation sẽ tự làm mới data
- QA phải cover case release xong quay lại list và item phải nằm đúng tab `Đã giải ngân`
...
5. Wire detail page
6. Thêm cơ chế refresh/invalidation cho list sau release
7. Test case:
   - mentorship pending payout
   - mentorship đã release
   - click release thành công
   - click release khi BE trả `PAYOUT_ALREADY_RELEASED`
   - release xong quay lại list và item chuyển đúng sang tab `Đã giải ngân`
...
- Không duplicate `GetAdvisorMentorshipById` ở service khác chỉ để phục vụ payment ops
